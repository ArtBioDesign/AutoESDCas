...
def run_blast(genome_path, workdir, blast_input_file_path, blast_genome_library_path):
    ...
    for i,row in df.iterrows():
        ...
        middle_off_target_df, high_off_target_df,low_off_target_df, aim_df = su.blast_output_evaluate(seq_len,genome_path, workdir, blast_input = os.path.join(workdir,input_fasta), blast_output= blast_output_file_path)
...
def blastEvaluate_uha_dha_in_genome(genome, uha_dha_df, workdir, id, region, name1, name2, name):
    ...
    middle_df["off target"] = "low"
    high_df["off target"] = "low"
...
def create_plasmid_order(uha_dha_sgRNA_df, name, plasmid_type):
        ...
        # list(plasmid_order_df.columns)
        plasmid_order_df = plasmid_order_df.rename(columns={"SEQUENCE": "Sequence synthesized",
                                                            "SEQUENCE SIZE(bp)": "Sequence synthesized size (bp)",
                                                            "IS OR NOT OPTIMIZED": "Whether codon optimization",
                                                            "PLASMID VECTOR": "Plasmid",
                                                            "PLASMID SEQUENCE": "Plasmid sequence",
                                                            "PLASMID SEQUENCE SIZE(bp)":"Plasmid sequence size (bp)"}
                                                            )
...
def one_plasmid_system_pcr_design_primer(gb_path,
    ...
    #分别提取，修饰后的uha、dha引物
    in_col = ['Name','Region',"primer_f_seq_(5'-3')_joint","primer_f_Tm","primer_r_seq_(5'-3')_joint","primer_r_Tm",'product_value_joint','product_size_joint']
    ou_col = ['Name','Region',"u_primer_f_seq_(5'-3')","u_primer_f_Tm","u_primer_r_seq_(5'-3')","u_primer_r_Tm",'UHA','UHA_size',"d_primer_f_seq_(5'-3')","d_primer_f_Tm","d_primer_r_seq_(5'-3')","d_primer_r_Tm",'DHA','DHA_size']
    ...
    if len(n20down_primer_df)>0:
        n20down_primer_p_df = n20down_primer_df[["Region","primer_f_seq_(5'-3')_joint","primer_f_Tm","primer_r_seq_(5'-3')_joint","primer_r_Tm",'product_value_joint','product_size_joint']]
    n20up_primer_p_df = pd.DataFrame()    
    if len(n20up_primer_df)>0:
        n20up_primer_p_df = n20up_primer_df[["Region","primer_f_seq_(5'-3')_joint","primer_f_Tm","primer_r_seq_(5'-3')_joint","primer_r_Tm",'product_value_joint','product_size_joint']]

    if len(seq_altered_primer_df)>0:
        seq_altered_primer_df = seq_altered_primer_df[["Region","primer_f_seq_(5'-3')_joint","primer_f_Tm","primer_r_seq_(5'-3')_joint","primer_r_Tm",'product_value_joint','product_size_joint']]
...
def two_plasmid_system_design_by_user_ccdb_region(uha_dha_sgRNA_df,no_sgRNA_plasmid,ccdb_plasmid_seq,ccdb_plasmid_region_seq,ccdb_region_json, enzyme_df, enzyme_name):
        ...
        if state == 'success':
            ...
            failture_ccdb_plasmid_primer_df = pd.DataFrame(ccdb_plasmid_primer_result_list)
...
def execute_one_plasmid_system_pcr(
    ...
    if plasmid_primer_desgin_type == 2:      
        ...
        # n20down_primer_p_df["Region"] = n20down_primer_p_df["Region"] + ";1"
    ...
    unique_all_primer_df = order.merge_primer(uha_primer_df, dha_primer_df, n20up_primer_p_df, n20down_primer_p_df.copy(), seq_altered_p_df, plasmid_sequencing_primer_df, genome_sequencing_primer_df)
    ...
    off_target_g_primer_df.to_csv(  os.path.join( dirname(xlsx_file),"Primer_g_offTarget.csv" ), index=False )

    off_target_g_primer_df = sr.off_target_primer(genome_sequencing_primer_df, off_target_g_primer_df, genome_path)     #修改回去 
 
   
    #10. 输出成功引物excel文件  
    with pd.ExcelWriter(xlsx_file) as writer:  
        uha_primer_df.to_excel(writer,sheet_name = 'Primer_UHA', index=False)
        dha_primer_df.to_excel(writer,sheet_name = 'Primer_DHA', index=False)
        seq_altered_p_df.to_excel(writer,sheet_name = 'Primer_inserted_fragment', index=False)
        n20up_primer_p_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment', index=False)  
        n20down_primer_p_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone', index=False) 
        plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1', index=False)
        genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G', index=False)

        # off_target_q_primer_df.to_excel(writer,sheet_name = 'Primer_p_offTarget')  
        off_target_g_primer_df.to_excel(writer,sheet_name = 'Primer_g_offTarget', index=False)
        unique_all_primer_df.to_excel(writer,sheet_name = 'Primer_order', index=False)
    ...
    with pd.ExcelWriter(xlsx_F_file) as writer: 
        failture_uha_primer_df.to_excel(writer,sheet_name = 'Primer_UHA')
        failture_dha_primer_df.to_excel(writer,sheet_name = 'Primer_DHA')
        failture_seq_altered_primer_df.to_excel(writer,sheet_name = 'Primer_inserted_fragment')
        failture_n20up_primer_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment')
        failture_n20down_primer_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone') 
        failture_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1')
        failture_genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G') 
...
def execute_one_plasmid_system_synthesis(   genome_path,
    ...
    unique_all_primer_df = unique_all_primer_df.rename(columns={'index':'ID','board_num':'index','hole':'Well',"primer":"Primer","index":"Index","base_num":"Number of bases"})
    # order_df = unique_all_primer_df[['index', 'hole', 'primer','base_num']]
    # order.create_orders(order_df, orders_path = order_file)       
    ...
    off_target_g_primer_df = sr.off_target_primer(genome_sequencing_primer_df, off_target_g_primer_df, genome_path)                                                                                                                                                   
    with pd.ExcelWriter(xlsx_file) as writer:  
        plasmid_sequencing_primer_df.to_excel(writer, sheet_name = 'Test_primer_P1', index = False)
        genome_sequencing_primer_df.to_excel(writer, sheet_name = 'Test_primer_G', index = False)
        # off_target_q_primer_df.to_excel(writer,sheet_name = 'Primer_p_offTarget')
        off_target_g_primer_df.to_excel(writer,sheet_name = 'Primer_g_offTarget')
        unique_all_primer_df.to_excel(writer, sheet_name = 'Primer_order', index = False)
    plasmid_order_df.to_excel(xlsx_order_file, index = False)
...
def execute_two_plasmid_system_synthesis(
    ...
    #成功
    with pd.ExcelWriter(xlsx_file) as writer:  
        sgRNA_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1', index=False)  
        ccdb_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P2', index=False)
        genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G', index=False)  

        # sgRNA_offtarget_primer.to_excel(writer,sheet_name = 'Offtarget_Test_primer_P1', index=False)
        # ccdb_offtarget_primer.to_excel(writer,sheet_name = 'Offtarget_Test_primer_P2', index=False) 
        off_target_g_primer_df.to_excel(writer,sheet_name = 'Offtarget_Test_primer_G', index=False)
        unique_all_primer_df.to_excel(writer, sheet_name = 'Primer_order', index = False)

    with pd.ExcelWriter(xlsx_order_file) as writer:
        ccdb_plasmid_order_df.to_excel(writer, sheet_name = 'HR_Plasmid_synthesis_order', index=False)
        sgRNA_plasmid_order_df.to_excel(writer, sheet_name = 'SgRNA_Plasmid_synthesis_order', index=False)  


    #失败
    with pd.ExcelWriter(xlsx_F_file) as writer: 
            failture_sgRNA_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1', index=False)
            failture_ccdb_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P2', index=False)
            # failture_genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G') 

    #复制one_plasmid_system_pcr_gb到one_plasmid_system_result中
    os.system(f"cp -r {os.path.join(parent_output,'two_plasmid_system_plasmid_sequencing_gb')} {output}")
    os.system(f"cp -r {os.path.join(parent_output,'two_plasmid_system_genome_sequencing_gb')} {output}")  
...
def execute_two_plasmid_system_pcr( 
    ...
    if  plasmid_primer_desgin_type == 1 and method == 'PCR':
        ...
        sgRNA_plasmid_primer_df, ccdb_plasmid_primer_df, failture_sgRNA_plasmid_primer_df, failture_ccdb_plasmid_primer_df = two_plasmid_system_design_by_user_region(
                                                                                                                                ...
                                                                                                                                sgRNA_region_json,
                                                                                                                                ccdb_region_json)
        # print("用户指定范围的pcr", sgRNA_plasmid_primer_df, ccdb_plasmid_primer_df, failture_sgRNA_plasmid_primer_df, failture_ccdb_plasmid_primer_df)
    ...
    #标准化，重命名
    # print( "PCR进入哪个分支" ,len(sgRNA_plasmid_p_df), len(ccdb_plasmid_p_df), len(seq_altered_p_df)  )
    if method == 'PCR':
        if len(sgRNA_plasmid_p_df) > 0 and len(ccdb_plasmid_p_df)>0 and len(seq_altered_p_df)>0:
            ...
            seq_altered_p_df = seq_altered_primer_df
            sgRNA_plasmid_p_df = sgRNA_plasmid_primer_df
        elif  len(seq_altered_p_df) > 0 and len(sgRNA_plasmid_p_df)==0 and len(seq_altered_p_df)==0:
            df_common_list = su.rename_common_primer_df(ccdb_plasmid_p_df)
            seq_altered_p_df = df_common_list[0]
            ccdb_plasmid_p_df = ccdb_plasmid_p_df
            sgRNA_plasmid_p_df = sgRNA_plasmid_primer_df
    ...
    if len(sgRNA_plasmid_sequencing_primer_template) > 0:
        ...
        sgRNA_plasmid_sequencing_primer_template = sgRNA_plasmid_sequencing_primer_template[['Region','plasmid','promoter_N20_terminator']].rename(columns={'Region':'ID',
                                                                                                                                                            'plasmid':"PLASMID",
                                                                                                                                                            "promoter_N20_terminator":"PROMOTER_N20_TERMINATOR"})
        if method == 'PCR':
            if len(sgRNA_plasmid_p_df) >0 :
                ...
                print("错误, sgRNA_plasmid_p_df没生成")

                sgRNA_plasmid_primer = pd.DataFrame(columns=['ID', 'PRIMER_LEFT_WHOLE_SEQUENCE', 'PRIMER_RIGHT_WHOLE_SEQUENCE'])
                type = 'sgRNA'
                sgRNA_pcr_tsv_df = p_d_seq.create_gb_for_region(sgRNA_plasmid_sequencing_primer_template, sgRNA_plasmid_primer, joint_len, cut_seq_len, gb_output, type)
    ...
    if method == 'PCR':
        ...
        with pd.ExcelWriter(xlsx_file) as writer:  
            ...
            dha_primer_df.to_excel(writer,sheet_name = 'Primer_DHA', index=False)
            seq_altered_p_df.to_excel(writer,sheet_name = 'Primer_inserted_fragment', index=False)
            sgRNA_plasmid_p_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment', index=False)  
            ccdb_plasmid_p_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone', index=False)
        ...
        #失败
        with pd.ExcelWriter(xlsx_F_file) as writer: 
            failture_uha_primer_df.to_excel(writer,sheet_name = 'Primer_UHA', index=False)
            failture_dha_primer_df.to_excel(writer,sheet_name = 'Primer_DHA', index=False)
            failture_seq_altered_primer_df.to_excel(writer,sheet_name = 'Primer_inserted_fragment', index=False)
            failture_sgRNA_plasmid_primer_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment', index=False)
            failture_ccdb_plasmid_primer_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone', index=False)

            failture_sgRNA_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1', index=False)
            failture_ccdb_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P2', index=False)
            failture_genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G', index=False) 
    else:
        #成功
        with pd.ExcelWriter(xlsx_file) as writer:  
            uha_primer_df.to_excel(writer,sheet_name = 'Primer_UHA', index=False)
            dha_primer_df.to_excel(writer,sheet_name = 'Primer_DHA', index=False)
            seq_altered_p_df.to_excel(writer,sheet_name = 'Primer_inserted_fragment', index=False)
            enzymeCutSeq_and_N20_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment', index=False)  
            ccdb_plasmid_p_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone', index=False)  
            sgRNA_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P1', index=False)
            ccdb_plasmid_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_P2', index=False)
            genome_sequencing_primer_df.to_excel(writer,sheet_name = 'Test_primer_G', index=False)
        ...
        if method == 'OLIGO':
            ...
            with pd.ExcelWriter(xlsx_F_file) as writer: 
                ...
                # failture_sgRNA_plasmid_primer_df.to_excel(writer,sheet_name = 'Primer_sgRNA_fragment')
                failture_ccdb_plasmid_primer_df.to_excel(writer,sheet_name = 'Primer_plasmid_backbone', index=False)
...
def main(data):
    ...
    # info = info[:10]  CM002240.1_52_deletion;CM002240.1:709042-709125      AM270990.1_38_substitution
    # info = info[info["name"]=="AM270998.1_42_deletion"]     
...
if __name__ == '__main__':
    ...
    if  call_method == 1:
        ...
        data1 = {
            ...
            "edit_sequence_design_workdir":"/home/yanghe/tmp/edit_sequence_design/Pseudomonas_aeruginosa/output/",
            "ref_genome":"/home/yanghe/program/data_preprocessing/temp/Pseudomonas_aeruginosa.fna",
        ...
        data2 = {
            "chopchop_input": "/home/yanghe/tmp/data_preprocessing/output/info_input.csv",   
            "sgRNA_result_path": "/home/yanghe/tmp/chopchop/output/sgRNA.csv",
            "edit_sequence_design_workdir":"/home/yanghe/tmp/edit_sequence_design/145_HR_new/output", 
            # "edit_sequence_design_workdir":"/home/yanghe/tmp/edit_sequence_design/opt_after/output",        
            "analysis":False,  
            "ref_genome": "/home/yanghe/tmp/data_preprocessing/output/xxx.fna",  #"/home/yanghe/tmp/data_preprocessing/output/xxx.fna",
            # "one_plasmid_file_path":"/home/yanghe/program/edit_sequence_design/input/pXMJ19-Cas9A-gRNA-crtYEb-Ts - ori .gb", 
            "one_plasmid_file_path":"/home/yanghe/program/edit_sequence_design/input/pCas9.gb",     
            "bowtie_path":"/home/yanghe/software/bowtie",   
            "no_ccdb_plasmid":"",
            "no_sgRNA_plasmid":"",   
            'sgRNA_result':{},  
        ...
        data3 = {     
            "chopchop_input": "/home/yanghe/program/edit_sequence_design/input/only_primer/only_primer_cor.csv",   
            "sgRNA_result_path": "",
            ...
            "bowtie_path":"/home/yanghe/software/bowtie",   
            "one_plasmid_file_path":"",   
            "no_ccdb_plasmid":"/home/yanghe/program/edit_sequence_design/input/only_primer/Plasmid_1.gb",
            "no_sgRNA_plasmid":"/home/yanghe/program/edit_sequence_design/input/only_primer/Plasmid_2.gb",
            ...
            "sgRNA_primer_json":{
                # "primer":"gcgtgcagggccagactgga"  
            },
            "ccdb_primer_json":{
                # "primer":"tcgattgcgtggataaggcta"
            },   
            "sgRNA_region_label":"region1",
            "ccdb_region_label":"region2",      
        ...
        data5 = {     
                    "chopchop_input": "/home/yanghe/program/edit_sequence_design/input/only_primer/info_input.csv",   
                    "sgRNA_result_path": "/home/yanghe/tmp/chopchop/output/sgRNA.csv",
                    "edit_sequence_design_workdir": "/home/yanghe/tmp/edit_sequence_design/output/",
                    "ref_genome": "/home/yanghe/program/edit_sequence_design/input/only_primer/GCA_000011325.1_ASM1132v1_genomic.fna",
                    "one_plasmid_file_path": "",  
