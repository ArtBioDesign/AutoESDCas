...
def primer_design(seqId,
    ...
    #检查引物设计模板序列
    seqTemplate = seqTemplate.replace(" ", "")
    seqTemplate = seqTemplate.replace("N", "A")
    seqTemplate = seqTemplate.replace("S", "A")
    seqTemplate = seqTemplate.replace("M", "A")   
    seqTemplate = seqTemplate.replace("K", "A")   

    # import re
    # pattern = re.compile('[^AGCT]', re.IGNORECASE)
    # replaced_seq = pattern.sub('A', seqTemplate)
    # print(replaced_seq)  # 输出替换后的序列
    ...
    if primer_type == 'plasmid_seq':
        ...
        if config.ENV == "opt_after":
            global_args['PRIMER_NUM_RETURN'] = 1000
        elif config.ENV == "opt_before":
            global_args['PRIMER_NUM_RETURN'] = 1000
...
def rename_u_d_primer_df(uha_primer_df,dha_primer_df):
    uha_primer_df = uha_primer_df.rename(columns={'Region':'ID',
                                              "primer_f_seq_(5'-3')":"PRIMER_LEFT_WHOLE_SEQUENCE",
                                              "u_primer_f_Tm":"PRIMER_LEFT_WHOLE_SEQUENCE_TM",
                                              "primer_r_seq_(5'-3')":"PRIMER_RIGHT_WHOLE_SEQUENCE",
                                              "u_primer_r_Tm":"PRIMER_RIGHT_WHOLE_SEQUENCE_TM",
                                              "UHA":"PRODUCT_SEQUENCE",
                                              "UHA_size":"PRODUCT_WHOLE_LENGTH"
                             })
    ...
    dha_primer_df = dha_primer_df.rename(columns={'Region':'ID',
                                                "primer_f_seq_(5'-3')":"PRIMER_LEFT_WHOLE_SEQUENCE",
                                                "d_primer_f_Tm":"PRIMER_LEFT_WHOLE_SEQUENCE_TM",
                                                "primer_r_seq_(5'-3')":"PRIMER_RIGHT_WHOLE_SEQUENCE",
                                                "d_primer_r_Tm":"PRIMER_RIGHT_WHOLE_SEQUENCE_TM",
                                                "DHA":"PRODUCT_SEQUENCE",
                                                "DHA_size":"PRODUCT_WHOLE_LENGTH"
                                })
...
def rename_common_primer_df(*df_list):
    ...
    for df in df_list:
        if 'index' not in df.columns:  
            df = df.rename(columns={'Region':'ID',
                                "primer_f_seq_(5'-3')_joint":"PRIMER_LEFT_WHOLE_SEQUENCE",
                                "primer_f_Tm":"PRIMER_LEFT_WHOLE_SEQUENCE_TM",
                                "primer_r_seq_(5'-3')_joint":"PRIMER_RIGHT_WHOLE_SEQUENCE",
                                "primer_r_Tm":"PRIMER_RIGHT_WHOLE_SEQUENCE_TM",
                                "product_value_joint":"PRODUCT_SEQUENCE",
                                "product_size_joint":"PRODUCT_WHOLE_LENGTH",
                    })
...
def bowtie_seq_genome(tmp_path, path, index_prefix, fasta_filename):
    ...
    if not exists(index_prefix):
        ...
        print(cmd)
        os.system(cmd)  
    # os.chmod(f'{config.BOWTIE_PATH}bowtie', 0o755) --norc
...
def convert_df_to_fastaFile(genome_path,df,id,name,input_fasta,lib_fasta):
    ...
    for i,row in df.iterrows():
        ...
        seqid = seqid.split(";")[0].replace(chrom+"_", "") + ";" + seqid.split(";")[1] + ";" + seqid.split(";")[2]
...
def blast_ha(ref_genome, blast_input_file_path, blast_output_file_path):  
    ...
    ref_lib = os.path.join( config.workdir, ref_lib )
    seq_length=0
...
def blast_output_evaluate(seq_len, genome_path , workdir, blast_input, blast_output):   
...
def get_gene_info_by_coordinates(gb_file, chromosome, start, end):
    ...
    for record in records:
        for feature in record.features:
            if feature.type == "gene":
                ...
                # 检查基因是否包含给定坐标
                if (
                    feature.location.start <= start <= feature.location.end
                    or feature.location.start <= end <= feature.location.end
                ):
                    gene_id = feature.qualifiers.get("gene", ["Unknown Gene"])[0]
                    print(f"Gene ID: {gene_id}")
                    print(f"Chromosome: {chromosome}")
                    print(f"Start position: {feature_start}")
                    print(f"End position: {feature_end}")
                    return

    print("No gene found at the specified coordinates.")

def extract_sequence_by_coordinates(gb_file, chromosome, start, end):
    records = SeqIO.parse(gb_file, "genbank")

    for record in records:
        if chromosome in record.id:
            sequence = record.seq[start-1:end]  # 注意：坐标是1-based，而 Python 的序列是0-based
            return sequence

    print("Chromosome not found in the GenBank file.")
    return None

def get_gene_info_from_gb(gb_file, feature_type):
    records = SeqIO.parse(gb_file, "genbank")
    ...
    for record in records:
        for feature in record.features:
            if feature.type == feature_type:
                gene_id = feature.qualifiers.get("gene", [])
    ...
    df.insert(3, 'Up region', 0)
    df.insert(4, 'Down region', 0)
    df.insert(5, 'Inserted sequence', '-')
    df.insert(6, 'Manipulation type', 'deletion')
