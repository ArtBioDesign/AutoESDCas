...
def get_invalid_primer(invalid_priemr_df):
    ...
    for i,v in invalid_priemr_df.iterrows(): 
        ...
        if False\
            and v["ID"] !=  "invalid_primer:NC_002516.2_44_substitution;NC_002516.2:3600412-3600477:SEQUENCING_PRIMER_1_AND_SEQUENCING_PRIMER_2:off_target" \
            ...
            try:
                ...
                off_target = eval(",".join(v["off_target"].split(",")[:-1])+"}")
    ...
    for i,v in SEQUENCING_PRIMER_1_SEQUENCING_PRIMER_2.iterrows():
        ...
        if pd.isna(v["After optimization SEQUENCING_PRIMER_1 location (start-end)"]):
            ...
            for k,value in cor.items():
                if len(value.split(";")) == 1 and "SEQUENCING_PRIMER_1" in k:
        ...
        if pd.isna(v["After optimization SEQUENCING_PRIMER_2 location (start-end)"]):
            ...
            for k,value in cor.items():
                if  len(value.split(";")) == 1 and "SEQUENCING_PRIMER_2" in k:
        ...
        if v["type (B: Before optimization);(A:After optimization)"] != opt_type_value:
                ...
                if pd.isna( v[f"{opt_type} optimization SEQUENCING_PRIMER_1 location (start-end)"] ) or  v[f"{opt_type} optimization SEQUENCING_PRIMER_1 location (start-end)"] == "":
                    ...
                    if len( result_df ) >0:
                        ...
                        invalid_primer.loc[i,f"{opt_type} optimization SEQUENCING_PRIMER_1 location (start-end)"] = ""
                ...
                if pd.isna(  v[f"{opt_type} optimization SEQUENCING_PRIMER_2 location (start-end)"]  ) or  v[f"{opt_type} optimization SEQUENCING_PRIMER_2 location (start-end)"]=="":
                    ...
                    if len( result_df ) >0:
                        ...
                        invalid_primer.loc[i,f"{opt_type} optimization SEQUENCING_PRIMER_2 location (start-end)"] = ""

    return invalid_primer

def extract_df(genome, index, opt_type):  

    edit_result_path = f"/home/yanghe/program/data_preprocessing/genome_test/{genome}/{index}/opt_{opt_type}_whole_process_output/edit_sequence_desgin/one_plasmid_system_result/Design_result.xlsx"
    one_plasmid_design_F_Result_path = f"/home/yanghe/program/data_preprocessing/genome_test/{genome}/{index}/opt_{opt_type}_whole_process_output/edit_sequence_desgin/one_plasmid_system_result/Failed_task.xlsx"
    ...
    Primer_g_offTarget =  pd.read_csv(os.path.join(dirname( edit_result_path ), "Primer_g_offTarget.csv") )  
...
def count_time(genome_name_list, columns_index, opt_type="before" ):
    ...
    for genome_name in genome_name_list:
        # opt_type = "before"
        genome_path = f"/home/yanghe/program/data_preprocessing/genome_test/{genome_name}_{opt_type}.csv"
...
def drawing_line_chart():
    ...
    names = ['Yarrowia_lipolytica_CLIB_122','Neurospora_crassa_OR74A','Schizosaccharomyces_pombe_972h-', 'Aspergillus_niger', 'Saccharomyces_cerevisiae_S288C'] 
    ...
    for genome in genome_name_list:  
        for i in range(50,550,50):   
                for step in range(1,6,1):
                        ...
                        print(genome,index)
                    # genome  = "Salmonella_typhimurium"    250-3
                    ...
                    #     pass
                        ...
                        path = f"/home/yanghe/program/data_preprocessing/genome_test/{genome}/{index}"
...
def save_input_file(xlsx_file="data_preprocessing/genome_test/all_input.xlsx"):
    ...
    # genome_name_list  =  ['Escherichia_coli','Corynebacterium_glutamicum', 'Pseudomonas_aeruginosa', 'Bacillus_subtilis', 'Salmonella_typhimurium']
    # ,'Neurospora_crassa_OR74A','Schizosaccharomyces_pombe_972h-', 'Saccharomyces_cerevisiae_S288C','Aspergillus_niger'
    genome_name_list = ['Neurospora_crassa_OR74A']   
    #
    genome_dict = {}
    for genome in genome_name_list: 
        ...
        for i in range(50,550,50):   
            for step in range(1,6,1):
                index = str(i)+"-"+str(step)
                genome_index_path = f"/home/yanghe/program/data_preprocessing/genome_test/{genome}/{index}/{index}.csv"
...
def save_sample(xlsx_file="/home/yanghe/program/data_preprocessing/genome_test/all_input.xlsx"):

    #genome_name_list  =  ['Escherichia_coli','Corynebacterium_glutamicum', 'Pseudomonas_aeruginosa', 'Bacillus_subtilis', 'Salmonella_typhimurium']
    # ,'Neurospora_crassa_OR74A','Schizosaccharomyces_pombe_972h-', 'Saccharomyces_cerevisiae_S288C','Aspergillus_niger'
    genome_name_list = ['Aspergillus_niger','Saccharomyces_cerevisiae_S288C','Neurospora_crassa_OR74A'] 
    #   
    genome_dict = {}
    ...
    xlsx_file="/home/yanghe/program/data_preprocessing/genome_test/all_input.xlsx"
    with pd.ExcelWriter(xlsx_file) as writer:
            for k,v in genome_dict.items():
                v.to_excel(writer,sheet_name = k,index_label='No.')
...
def main():  
    #分析结果的路径
    analysis_path =  "/home/yanghe/program/test/analysis/"
    ...
    ## ,'Neurospora_crassa_OR74A','Schizosaccharomyces_pombe_972h-',, 'Aspergillus_niger'     ##满足条件的物种 'Saccharomyces_cerevisiae_S288C'
    # ['Yarrowia_lipolytica_CLIB_122.fna','Neurospora_crassa_OR74A.fna','Schizosaccharomyces_pombe_972h-.fna', 'Aspergillus_niger.fna', 'Saccharomyces_cerevisiae_S288C.fna']
    genome_name_list = ['Aspergillus_niger']             #S'Aspergillus_niger','Neurospora_crassa_OR74A'  
